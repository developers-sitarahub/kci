...
export default function ForSalePage() {
  ...
  useEffect(() => {
    fetch("http://127.0.0.1:8000/web-services/properties/?property_type=FOR_SALE")
      .then((res) => res.json())
      .then((data: any[]) => {
        setActiveProperties(data.filter((p) => p.status === "ACTIVE"));
        setSoldProperties(data.filter((p) => p.status === "SOLD"));
      })
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          >
            ...
            <p className="text-lg md:text-xl text-slate-300 leading-relaxed text-balance">
              Discover unparalleled commercial real estate investment
              opportunities spanning Houston and surrounding areas.
            </p>
          </motion.div>
        </div>
      </section>
